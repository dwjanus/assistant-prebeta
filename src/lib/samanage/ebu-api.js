  ...
  return {
    ...
    metrics (options) {
      return new Promise((resolve,reject) =>{
        ...
        console.log(`\n--> sfDate:\n${util.inspect(sfDate)}`)
        let sfDate2 = new jsforce.Date()
        console.log(`\n--> sfDate:\n${util.inspect(sfDate2)}`)
        let startClosedDate = sfDate.parseDate(options.DatePeriod.split('/')[0])
        let endClosedDate = sfDate.parseDate(options.DatePeriod.split('/')[1])
        let statusDateType = ''
        ...
        conn.sobject('Case')
        .find(searchParams, returnParams) // need handler for if no number and going by latest or something
        .where(`${statusTypeDate} >= ${startClosedDate} AND ${statusTypeDate} <= ${endClosedDate}`)
        .sort('-LastModifiedDate')
        .execute((err, records) => {
          if (err) return reject(err)
          let sample_records = records.slice(0, 5) // Show first 5 records
